...
type B = { id: number; code: string; product_name?: string; oven_label?: string; temp_tier?: string; preheat_min?: number; start_min: number; ferment_end?: number; bake_end?: number; status: string };
...
export default function BatchesPage() {
  ...
  return (<>
    ...
    <table className="table"><thead><tr><th>批次</th><th>产品</th><th>温度档</th><th>炉位</th><th>预热</th><th>发酵</th><th>烘烤结束</th><th>状态</th></tr></thead>
    <tbody>{rows.map(b => <tr key={b.id}><td className="mono">{b.code}</td><td>{b.product_name}</td><td>{b.temp_tier ?? "—"}</td><td>{b.oven_label}</td>
      <td className="mono">{b.preheat_min ? `${fmt(b.start_min - b.preheat_min)}–${fmt(b.start_min)}` : "—"}</td>
