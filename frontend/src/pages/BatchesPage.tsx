...
type B = { id: number; code: string; product_name?: string; oven_label?: string; temp_profile?: string; preheat_min?: number; start_min: number; ferment_end?: number; bake_end?: number; status: string };
...
export default function BatchesPage() {
  ...
  return (<>
    ...
    <table className="table"><thead><tr><th>批次</th><th>产品</th><th>温度档</th><th>炉位</th><th>发酵</th><th>烘烤结束</th><th>状态</th></tr></thead>
    <tbody>{rows.map(b => <tr key={b.id}><td className="mono">{b.code}</td><td>{b.product_name}</td>
      <td>{b.temp_profile}{(b.preheat_min ?? 0) > 0 && <span className="muted">（预热 {b.preheat_min}′）</span>}</td><td>{b.oven_label}</td>
